...
func _ready():
	...
	equip_slots[3].slot_type = SlotClass.SlotType.ONHAND
	equip_slots[4].slot_type = SlotClass.SlotType.OFFHAND
...
func able_to_put_into_slot(slot):
	...
	elif slot.slot_type == SlotClass.SlotType.ONHAND:
		return holding_item_category == "on_hand"
	elif slot.slot_type == SlotClass.SlotType.OFFHAND:
		return holding_item_category == "off_hand"
	return true
